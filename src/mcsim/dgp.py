...
This module holds the true models that generate the simulated time series, the
plain VAR, the VARMA, and the nonlinear lag-quadratic VAR, together with the
closed-form impulse responses of those models. The impulse responses are the
targets that the estimators are scored against.
"""
...
def var_ma_matrices(A: np.ndarray, horizon: int) -> np.ndarray:
    ...
    Psi_0 = I, and Psi_h = sum_{i=1}^{min(h,p)} A_i Psi_{h-i}.
    """
    A = np.asarray(A, dtype=float)
    p, k, _ = A.shape
    psi = [np.eye(k)]
...
# ---------------------------------------------------------------------------
# VARMA(p, q), the VAR baseline augmented with a moving-average component
# ---------------------------------------------------------------------------
#
...
# component is a misspecification of their VAR-only Wold approximation.
...
class VARMASpec:
    ...
    Theta : array (q, K, K)
        ...
        innovations u_t. Theta = 0 (q = 0) nests the baseline VARSpec.
...
def varma_ma_matrices(A: np.ndarray, Theta: np.ndarray, horizon: int) -> np.ndarray:
    ...
    The Wold coefficients solve A(L) Phi(L) = Theta(L). Matching the coefficient
    of L^h in (I - sum_i A_i L^i)(sum_h Phi_h L^h) = (I + sum_j Theta_j L^j) gives

        Phi_0 = I,
        Phi_h = sum_{i=1}^{min(h,p)} A_i Phi_{h-i}  +  Theta_h     (Theta_h = 0 for h > q).

    This is the pure-VAR recursion of var_ma_matrices with the extra additive MA
    term Theta_h at horizons 1..q.
    """
    A = np.asarray(A, dtype=float)
...
# ---------------------------------------------------------------------------
# Stationary covariance of a (linear) VAR(p). This is used to standardise the
...
def stationary_cov(A: np.ndarray, B: np.ndarray) -> np.ndarray:
    ...
    Q = np.zeros((m, m))
    Q[:k, :k] = B @ B.T
    # Column-stacking vec, so vec(C Gamma C') = (C kron C) vec(Gamma).
...
# ---------------------------------------------------------------------------
# VAR(p) with a quadratic lagged-state term (the nonlinear-DGP extension)
# ---------------------------------------------------------------------------
#
...
# centred, so gamma=0 nests the baseline VARSpec and the term stays order one in
...
# standardising tames it, and empirical stability is checked in the notebook.
...
class QuadLagVARSpec:
    ...
    ----------
    A : array (p, K, K)
        Autoregressive matrices of the linear component (as in VARSpec).
    B : array (K, K)
        Lower-triangular structural impact matrix, Sigma_u = B B'.
    gamma : float
        Nonlinearity strength. ``gamma = 0`` nests the baseline VARSpec.
    ...
    projection, which is the estimand that both linear LP and linear VAR target
    (Plagborg-Moller and Wright, Prop. 1), is therefore gamma-dependent and no
    longer equals the baseline IRF. The estimand has no closed form here, so it
    is computed by large-sample projection (see the notebook).
    """
...
def simulate_quad_lag_var(
    ...
    The lag-quadratic can be explosive for large gamma or high persistence. A
    diverging path can stay finite yet grow astronomically (for example 1e120),
    which silently corrupts the pooled moments, so any path whose magnitude
    exceeds ``diverge_cap`` (or goes non-finite) is returned as all-NaN. The MC
    driver then records that replication as an estimator failure rather than
    letting a runaway trajectory bias the results. The caller can report the
    failure rate (the empirical share of explosive paths) as a stationarity
    diagnostic.
    """
