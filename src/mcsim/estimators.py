...
def fit_var_ols(y: np.ndarray, p: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    (A_hat, sigma_u, resid)
        ...
        ``sigma_u`` is the (K, K) residual covariance; ``resid`` is the
    ...
    # coef rows 1.. hold lag blocks; A_i maps y_{t-i} -> y_t, so A_i = block.T.
...
def estimate_var_irf(
    ...
    """Fit VAR(p), recursively identify, and return the structural IRF.

    Reduced-form coefficients are estimated by OLS; the impact matrix is the
    Cholesky factor \\hat B = chol(\\hat Sigma_u); the structural IRF is
