...
As Local Projection inference gained in popularity over the last couple of years, more and more scholars have started comparing the asymptotical and finite sample trade-offs of both methods, such as \textcite{kilian2011reliable},  \textcite{bruns2022comparison}, \textcite{li2024local}, \textcite{olea2025local} among others. Their comparison have yielded these and that results. 


\textcite{plagborg2021local} were the first to show analytically, that both methods share the same population impulse response function under weak stationarity and unrestriced lag structure. Therefore any conventional LP impulse response can be obtrained through an appropriately ordered recursive VAR and vice versa. 
\textbf{maybe here the equation proof}
Furthermore, as both methods are best linear predictors of the underlying DGP, the authors state that both methods are symetrically robust to non-linear misspecification
