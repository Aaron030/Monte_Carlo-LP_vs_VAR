...
Impulse response functions can be nonparametically defined as the difference between two conditional forecasts of the outcome variable (e.g. Koop (1996)). Both forecasts are conditioned on the same history up to period $t-1$, but differ in the realization at time $t$. While one conditions on a shock of size $\delta$ to the variable of interest while the other one does not. The observed difference in the projected paths at horizon $h$ then identifies the causal effect of the shock in population:

\begin{equation}
    IR(t, h,\delta) \equiv \mathbb{E}\left[y_{t+h}|s_t = s_0 + \delta; \mathbf{x_t}\right] - \mathbb{E}\left[y_{t+h}|s_t = s_0; \mathbf{x_t}\right]
\end{equation}

Vector Autoregressions and Local Projections hereby serve as as two ways of estimating the in sample impulse responses. The first, introduced by \textcite{sims1980macroeconomics} does this and that, the latter established by \textcite{jorda2005estimation} by that and this. 

While \textcite{plagborg2021local} have shown that asymptotically VARs and LPs estimate the same impule responses, \textcite{li2024local}, \textcite{olea2025local} have demonstrated that both lie on the opposite ends of the bias-variance spectrum. However, more recently \textcite{ludwig2026local} has shown that the comparison of both methods is constrained when size of the lagged controls is fixed, resulting in an unfit comparison.
